    ...
    return apartment.save()
        ...
        const updateById = (req, res) => {
            ...
            Apartment.findById({ _id: req.params.idApartment })
              .then((apartment) => {
                if (apartment.idAdvertiser != _id) {
                  return res
                    .status(400)
                    .send({ message: "Cannot update this apartment!" });
                }

                const myapartment = {  name, description,address,numOfBad, image, cost, } = req.body;

                Apartment.findByIdAndUpdate(id, myapartment, { new: true })
...
const deleteById= (req, res) => {
    ...
    Apartment.findById({ _id: req.params.idApartment })
        .then((apartment) => {
            if (a.idAdvertiser != req.params.id)
            return res
              .status(400)
