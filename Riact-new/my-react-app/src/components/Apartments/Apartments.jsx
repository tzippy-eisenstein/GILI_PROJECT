...
//     debugger
//     deleteApartment(_id ,{ headers: { 'Authorization': token } }
//     )
//         .then(response => {
//             Swal.fire({
//                 icon: 'success',
...
//   }
//     axios.patch(`http://localhost:3001/apartment/update/${currentAdvertiser._id}/${id}`,apartment,{ headers: { 'Authorization': token } })
//         .then((x) => {
//             Swal.fire({
//                 icon: 'success',
...
//               fullWidth
//           />
//           <button onClick={handlegetByPriceRange}>סנן לפי מחיר</button>
//           <button onClick={handlegetByNumBads}>סנן לפי מספר מיטות</button>
//           <button onClick={handleGetYourApartment}>קבל רק את הדירות שלך</button>
//       </div>
//       <div style={{ backgroundColor: '#252926', padding: '1rem' }}>
//           <Typography variant="h4" align="center" style={{ color: '#AF8C53', marginBottom: '0.5rem' }}>Properties for Sale</Typography>
//           <Grid container spacing={4}>
//               {apartments && apartments.map(item => (
//                   <Grid item xs={12} sm={6} md={4} lg={3} key={item.id}>
//                             <ApartmentCard
//                                 _id={item._id}
//                                 codeAdvertiser={item.idAdvertiser}
//                                 codeCategory={item.idCategory}
//                                 name={item.name}
//                                 addition={item.adition}
//                                 address={item.address}
//                                 numberBed={item.numOfBad}
//                                 price={item.cost}
//                                 codeCity={item.cityId}
//                                 image={item.image}
//                             ></ApartmentCard>
//                   </Grid>
//               ))}
...
import {getByPriceRange} from '../redux/apiApartments'
import {getByNumOfBads} from '../redux/apiApartments'
import { FormControl, InputLabel, MenuItem, Select, TextField, colors } from '@mui/material';
import Menu from '@mui/material/Menu';
import MoreVertIcon from '@mui/icons-material/MoreVert';
import { deleteApartment } from '../redux/apiApartments';
import { useSelector } from 'react-redux';
...
import { useNavigate } from 'react-router-dom';
import axios from 'axios';
import { Details, Token } from '@mui/icons-material';
...
const details = (_id) => {
  debugger
   nav(`/DetailsComp/${_id}`)
    ...
    <div style={{ marginTop: '35px' }}> {/* נוסיף את הערך הנכון ל margin-top */}
    {/* כאן הקומפוננטה הקיימת */}
...
<Box sx={{ mt: 0.5, mb: 2, ml: 2, mr: 2 }}>
  ...
  <ListItem>
    ...
    >
        <MenuItem onClick={() => details(apartment._id)}>more details</MenuItem>
    </Menu>
</div>
                               ...
                               {apartment.idCategory &&
                                ...
                                <Typography variant="body2" color="text.secondary" style={{ color: '#AF8C53'}}>
                                     ...
                                     <MenuItem onClick={() => details(apartment._id)} style={colors}>more details</MenuItem>
                                     </Typography>}
