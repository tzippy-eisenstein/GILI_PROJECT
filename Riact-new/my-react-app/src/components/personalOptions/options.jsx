...
export const  AccountMenu =()=> {
  ...
  const handleAddCategory = () => {
    ...
    debugger
        AddCategory(Category, { headers: { 'Authorization': token } })
...
const handleApartmentFormSubmit = async (event) => {
  ...
  axios.post(`http://localhost:3001/apartment/add/`, formData, { headers: { 'Authorization': token } })
      .then((x) => {
          Swal.fire({
              icon: 'success',
              title: 'Add Apartment',
...
const disConnect = () => {
  const c = localStorage.getItem("CurrentAdvertiser");
  const currentUser = JSON.parse(c);
  localStorage.setItem("token", null);
  // localStorage.setItem("tokenCustomer", null);
  localStorage.setItem("CurrentAdvertiser", null);
  swal(
    "Hello!",
    `disconnect ${currentAdvertiser.email} in successfully!`,
    "success"
  );
};
    ...
    <React.Fragment>
      ...
      >
        <MenuItem onClick={disConnect}>
          <Avatar /> disconnection
          </MenuItem>
        <Divider />
        <MenuItem onClick={handleAddApartment}><ListItemIcon><Settings fontSize="small" /></ListItemIcon>Add Apartment</MenuItem>
        <MenuItem onClick={handleAddCity}> <ListItemIcon><Logout fontSize="small" /></ListItemIcon> Add city </MenuItem>
        <MenuItem onClick={handleAddCategory}><AddCircleOutlineOutlinedIcon><PersonAdd fontSize="small" /></AddCircleOutlineOutlinedIcon>Add Category  </MenuItem>
